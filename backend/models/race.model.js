  ...
  async create(data) {
    ...
    const result = await pool.request()
      ...
      .query(`
        ...
        OUTPUT INSERTED.*
        VALUES (@nom, @prix_nourrit_g, @prix_vente_g, @prix_oeuf, @semaine_ponte, @duree_incubation)
      `);
    ...
    const result = await pool.request()
      ...
      .query(`
        UPDATE Race SET
          ...
          prix_oeuf = @prix_oeuf, semaine_ponte = @semaine_ponte, duree_incubation = @duree_incubation
        OUTPUT INSERTED.*
        WHERE race_id = @id
      `);
    return result.recordset[0] || null;
